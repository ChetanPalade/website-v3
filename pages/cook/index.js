...
import cookData from '../../context/prices.json';
...
const Cook = () => {
  ...
  const [totalPeople, setTotalPeople] = useState('1 People');
  ...
  const options = ['Breakfast', 'Lunch', 'Dinner', 'All 3 meals'];
  const { getRootProps, getRadioProps } = useRadioGroup({
    name: 'Peoples',
    defaultValue: 'Breakfast',
    onChange: (value) => {
      const newValue = cookData['Cook'][planType][totalPeople][value];
      setPrice(newValue * (planType == 'One Time' ? 1.2 : 1.5));
      const previous = JSON.parse(localStorage.getItem('user'));
      const newData = { ...previous, meal: value };
      ...
      >
        <Box position="relative" w={{ md: '55%', base: '100%' }}>
          {!isMobile ? (
            ...
            <>
              <Img src="images/cookMobile.webp" alt="cookBook" />
              <Heading
                position="absolute"
                top="10%"
                left="15%"
          ...
          >
            <Formik
              initialValues={{
                ...
                message: '',
                people: { value: '1 People', label: '1 People' },
                vegNonveg: { value: 'Both', label: 'Both' },
              }}
              onSubmit={(values, actions) => {
                setTimeout(() => {
                  localStorage.setItem('user', JSON.stringify(values, null, 2));
                  setPlanType(values.plan);
                  setTotalPeople(values.people.value);
                  setPrice(
                    cookData['Cook'][values.plan][totalPeople]['Breakfast'] *
