...
import nannyData from '../../context/prices.json';
...
const Nanny = () => {
  ...
  const options = ['6 hours', '8 hours', '10 hours', '12 hours', '24 hours'];
  const langOptions = [
  ...
  const { getRootProps, getRadioProps } = useRadioGroup({
    ...
    onChange: (value) => {
      const newValue = nannyData['Nanny']['Monthly'][value];
      setPrice(newValue * 1.5);
      const previous = JSON.parse(localStorage.getItem('user'));
      const newData = { ...previous, hours: value };
      ...
      >
        <Box position="relative" w={{ md: '60%', base: '100%' }}>
          {!isMobile ? (
            ...
            <>
              <Img src="images/nannyMobile.webp" alt="nannyBook" />
              <Heading
                position="absolute"
                top="10%"
                left="12%"
          ...
          >
            <Formik
              initialValues={{
                ...
                message: '',
                ageGreaterthan2: { value: 1, label: 1 },
                ageLessthan2: { value: 0, label: 0 },
              }}
              onSubmit={(values, actions) => {
                setTimeout(() => {
                  localStorage.setItem('user', JSON.stringify(values, null, 2));
                  setPrice(nannyData['Nanny']['Monthly']['6 hours'] * 1.5);
            ...
            >
              {(props) => (
                <Form>
                  ...
                  {/* <Flex flexDirection={{ md: 'row', base: 'column-reverse' }}>
                    <Box w="100%">
                        ...
                        {({ field, form }) => (
                          <FormControl my="5%">
                            <Textarea
                              isDisabled={
                                !JSON.parse(
                                  form.values.specificPreference.value
                                )
                              }
                      ...
                      <Field name="ageGreaterthan2">
                        {({ field, form }) => (
                          <FormControl
                            ...
                            alignItems="center"
                          >
                            <FormLabel>{'No. of Children (>2yrs):'}</FormLabel>
                            <Select
                              useBasicStyles
                              {...field}
                              options={numberofChild}
                              placeholder={`?`}
                              chakraStyles={customStyles}
                              onChange={(value) =>
                                form.setFieldValue(field.name, value)
